...
def update():
    ...
    if request.method=='POST':
        ...
        sql = "INSERT INTO id_caretaker(id,name,mobile,childname,uname) VALUES (%s, %s, %s, %s, %s)"
        val = (maxid,name,mobile,childname,uname)
        mycursor.execute(sql,val)
        mydb.commit()
        msg="ok"
...
def test_video():
    ...
    return render_template('test_video.html',msg=msg,vdata=vdata)
...
def process():
    ...
    mycursor.execute("SELECT * FROM id_caretaker where uname=%s order by id desc limit 0,1",(uname,))
    ...
    if detect=="1":
        ...
        mess="Infant Drowning"
        

            
    return render_template('process.html',name=name,mess=mess,mobile=mobile,name2=name2,mobile2=mobile2,sms=sms,s1=s1)
